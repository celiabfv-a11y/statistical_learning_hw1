...
This project focuses on analyzing population healthcare risk factors using various statistical and data analysis techniques.
Our dataset, “dirty_v3_path.csv”, contains 20 variables and 30,000 observations.

Among the 20 variables, we find:
  ...
  Physical Activity: Physical activity (hours per week).
...
Data preprocessing ensures that the dataset is clean, complete, and ready for analysis.
It allows for detecting errors, missing values, and outliers that may distort results.
Feature engineering steps (e.g., converting categorical variables such as gender into numeric form) are also included to make the data model-ready.
...
Exploratory visualization helps identify patterns, distributions, and relationships between variables before modeling.
For instance, a boxplot reveals extreme values (patients with long hospital stays), while the correlation matrix highlights relationships between variables such as glucose and triglycerides.
...
# hypertension and cancer are likely to suffer from this
...
Regression models are used to predict a numerical variable (here, LengthOfStay) from other predictors.
Resampling (bootstrap) evaluates model robustness and variability, identifying which model generalizes best.
In this case, Model 2 shows the lowest MSE, suggesting the best performance without overfitting.
...
PCA reduces dataset dimensionality by combining correlated variables into principal components.
This simplifies analysis and reveals latent axes (e.g., metabolic vs. lifestyle factors).
It also helps visualize patterns or separations among patient groups.
...
Factor Analysis identifies latent variables (factors) explaining correlations among observed variables.
For example, one factor may group metabolic indicators (glucose, cholesterol) and another lifestyle indicators (sleep, physical activity).
This method helps interpret the internal structure of the dataset.
...
Clustering groups individuals without prior labels, based on similarity among variables.
It reveals patient profiles (e.g., groups with higher metabolic risk or shorter hospital stays).
Different methods (K-means, PAM, Gaussian Mixture Models) offer complementary perspectives.
...
Preprocessing: The dataset contained missing values in clinical variables; these were removed, producing a complete dataset.

Visualization: LengthOfStay is asymmetric and contains outliers; strong correlations exist among metabolic variables.

Regression and Resampling: After 200 bootstraps, the larger model (M2) consistently showed a lower MSE than the simpler one; the polynomial model (M3) did not always improve performance → possible overfitting.

PCA: Two to three components explain most of the variability, separating metabolic from lifestyle dimensions.

FA: With three Varimax-rotated factors, we obtain an interpretable structure similar to PCA.

Clustering: With k = 4, distinct groups appear with different metabolic risks and average hospital stays → useful segmentation.
